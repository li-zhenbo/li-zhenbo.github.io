...
**核心规则：一次成功 = 立刻停止，禁止重复调用。**

- git push 返回 `main -> main` 即推送成功，立刻停止，不要再用 echo 确认
- 文件写入/编辑成功后不要再 Read 验证
- 命令返回预期结果后不要再重复执行
- 当用户说"继续"且无新任务时，直接问用户要做什么，不要做任何操作
- 如果之前的回答已经有明确的结论/完成状态，用户说"继续"只是希望你继续等待或进行下一项工作，**不要重复输出相同内容**
