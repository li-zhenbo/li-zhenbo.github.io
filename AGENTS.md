...
每次专注当前任务，不反复验证之前已确认的旧结果。处理新任务时只验证当前任务的直接产出。

**任务完成后立刻停止**：当工具返回成功消息（如 git push 成功、echo 确认完成），不要再重复调用同一个命令反复确认。收到第一次成功返回即任务结束，直接向用户汇报，不追加冗余验证。

**文件操作精确限定**：修改封面或图片时，只动目标文件（如只改 lxxb-cover.png 和 cpb-cover.png），不要批量操作所有同目录文件。避免误改前四篇等不需要改动的封面。
