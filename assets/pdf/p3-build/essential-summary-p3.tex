...
\label{eq:model}
\end{equation}
where $\lambda$, $\mu$, $\nu$ are stiffness parameters, and the right side contains a \textbf{6-term generalized nonlinear damping}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{ex2-potential.eps}
\caption{\textbf{Figure 6.} Potential portrait of the DHRL oscillator. The rich potential landscape enables homoclinic and heteroclinic bifurcations.}
\end{figure}
...
By dividing the integration interval into eight equal subintervals, the Fourier coefficients $p_{2i}$ become \textbf{explicit algebraic functions}, and the entire perturbation procedure now runs purely symbolically.
...
\end{equation}
where $V(x) = \int g(x)\,dx$ is the potential energy. For complicated $g(x)$, this expression is analytically intractable---which is precisely why the Simpson discretization is needed. After discretization, $\Phi_0$ is expanded as a Fourier series with $M=4$.
...
Setting the limit cycle amplitude equal to the saddle point coordinate in Eq.\ \eqref{eq:amplitude} yields the critical $\mu_c^{\mathrm{hom}}$ for homoclinic bifurcation. For heteroclinic bifurcation, setting the amplitude to both connected saddle point coordinates yields the critical $\mu_c^{\mathrm{hetero}}$.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ex1-muca.eps}
\caption{\textbf{Figure 2.} Evolution of limit cycles in the parametric plane ($\mu_c$--$A$ curve). The complete lifecycle is visible: four regions (I--IV) identified based on the number and stability of limit cycles. Semi-stable bifurcation points mark transitions between regions.}
\label{fig:fig2}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ex1-phase.eps}
\caption{\textbf{Figure 3.} Phase portraits corresponding to the four regions in Figure~2. The method accurately captures the transition from no limit cycle to stable, to coexisting stable+unstable, and back to stable-only behavior.}
\label{fig:fig3}
\end{figure}

Figure~2 reveals the complete limit cycle lifecycle: Region I (no limit cycle), Point A (semi-stable emergence), Region II (stable+unstable coexistence), Point B (second semi-stable bifurcation), Region III (one stable+one unstable), and Region IV (single stable cycle persists). Figure~3 provides the corresponding phase portraits in each region.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ex3-muca.eps}
\caption{\textbf{Figure 8.} Variation of the heteroclinic bifurcation parameter $\mu_c^{\mathrm{hetero}}$ with system parameters in different conditions. Each subfigure shows how $\mu_c^{\mathrm{hetero}}$ changes as one damping coefficient is varied. The MGHFP method enables this multi-parameter analysis analytically.}
\label{fig:fig8}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ex3-phase.eps}
\caption{\textbf{Figure 9.} Phase portraits of heteroclinic solutions with different parameters. Each subfigure compares the analytical MGHFP solution with the Runge--Kutta numerical reference, validating the method's accuracy for heteroclinic orbit prediction.}
\label{fig:fig9}
\end{figure}
...
\item \textbf{First purely symbolic GHFP variant:} The introduction of composite Simpson quadrature into the GHFP framework makes the proposed method the first variant capable of \textbf{purely symbolic execution}---no parameter pre-assignment needed. This is the foundational innovation enabling all subsequent MGHFP work.
...
\item \textbf{Homoclinic and heteroclinic bifurcations predicted (Figures 8--9):} Both types of global bifurcation are quantitatively analyzed. The analytical expression for $\mu_c^{\mathrm{hetero}}$ enables systematic study of its dependence on multiple damping coefficients simultaneously.

\item \textbf{Method validated on maximally challenging test case:} The DHRL oscillator combines rational restoring force $\frac{\lambda x + \mu x^3}{1+\nu x^2}$ with six-term generalized Rayleigh--Lienard damping. All analytical predictions agree with Runge--Kutta results.

\item \textbf{Foundation for the MGHFP series:} This paper establishes the framework subsequently extended to SD oscillators with quartic damping (IJNLM 2025, with Pade enhancement) and coupled SD oscillators (Phys.\ Scr.\ 2026).
\end{enumerate}
