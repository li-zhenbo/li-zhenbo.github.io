...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth,height=0.45\textheight,keepaspectratio]{p3-fig-p13.png}
\caption{\textbf{Figure 6.} Potential portrait of the DHRL oscillator. The rich potential landscape enables homoclinic and heteroclinic bifurcations, making this an ideal testbed for the MGHFP method.}
\end{figure}
...
\end{equation}
where $V(x) = \int g(x)\,dx$ is the potential energy. For complicated $g(x)$, this expression is analytically intractable --- which is precisely why the Simpson discretization is needed. After discretization, $\Phi_0$ is expanded as a Fourier series $\Phi_0(\varphi) = \sum_{i=0}^{M} (p_{2i}\cos 2i\varphi + q_{2i}\sin 2i\varphi)$ with $M$ chosen appropriately (here $M=4$).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth,height=0.45\textheight,keepaspectratio]{p3-fig2.png}
\caption{\textbf{Figure 2.} Evolution of limit cycles in the parametric plane ($\mu_c$--$A$ curve). The complete lifecycle is visible: four regions (I--IV) are identified based on the number and stability of limit cycles. Region I: no limit cycle; Region II: one stable cycle; Region III: two cycles (stable + unstable); Region IV: one stable cycle with different amplitude range. Semi-stable bifurcation points A and B mark the transitions between regions.}
\label{fig:fig2}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth,height=0.45\textheight,keepaspectratio]{p3-fig3.png}
\caption{\textbf{Figure 3.} Phase portraits corresponding to the four regions in Figure~2. Each subfigure shows the limit cycles in phase space for a representative $\mu_c$ value within that region. The method accurately captures the transition from no limit cycle to stable, to coexisting stable+unstable, and back to stable-only behavior.}
\label{fig:fig3}
\end{figure}

Figure~2 reveals the complete limit cycle lifecycle in parametric space:
\begin{itemize}[leftmargin=2em]
\item \textbf{Region I} ($\mu_c < \mu_c^A$): no limit cycle exists.
\item \textbf{At Point A}: a \textbf{semi-stable} limit cycle emerges (outer-stable, inner-unstable).
\item \textbf{Region II} ($\mu_c^A < \mu_c < \mu_c^B$): two coexisting limit cycles --- a smaller unstable cycle and a larger stable cycle.
\item \textbf{At Point B}: a second semi-stable bifurcation occurs.
\item \textbf{Region III} ($\mu_c^B < \mu_c < 0$): one stable and one unstable limit cycle coexist.
\item \textbf{Region IV} ($\mu_c > 0$): only a single stable limit cycle persists.
\end{itemize}

Figure~3 provides the corresponding phase portraits in each region, visually confirming the parametric analysis.

\section{Example 2: Heteroclinic Bifurcation (Section 4.3)}

The heteroclinic bifurcation analysis focuses on oscillator (68) from the paper, which involves a different parameter configuration. The key innovation here is the ability to derive \textbf{analytical expressions} for the heteroclinic bifurcation parameter $\mu_c^{\mathrm{hetero}}$, enabling quantitative study of its variation with respect to system parameters.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth,height=0.45\textheight,keepaspectratio]{p3-fig8.png}
\caption{\textbf{Figure 8.} Variation of the heteroclinic bifurcation parameter $\mu_c^{\mathrm{hetero}}$ with system parameters in different conditions. Each subfigure shows how $\mu_c^{\mathrm{hetero}}$ changes as one damping coefficient is varied while others are fixed. The MGHFP method enables this multi-parameter analysis analytically --- a capability classical GHFP lacks.}
\label{fig:fig8}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth,height=0.45\textheight,keepaspectratio]{p3-fig9.png}
\caption{\textbf{Figure 9.} Phase portraits of heteroclinic solutions with different parameters. Each subfigure compares the analytical MGHFP solution with the Runge--Kutta numerical reference. The excellent agreement validates the method's accuracy for heteroclinic orbit prediction.}
\label{fig:fig9}
\end{figure}
...
\item \textbf{First purely symbolic GHFP variant:} The introduction of composite Simpson quadrature into the GHFP framework makes the proposed method the first variant capable of \textbf{purely symbolic execution} for complicated oscillators --- no parameter pre-assignment needed. This is the foundational innovation that enables all subsequent MGHFP work.
...
\item \textbf{Homoclinic and heteroclinic bifurcations predicted (Figures 8--9):} Both types of global bifurcation are quantitatively analyzed. The analytical expression for $\mu_c^{\mathrm{hetero}}$ enables systematic study of its dependence on all damping coefficients simultaneously --- a multi-parameter capability unique to MGHFP.

\item \textbf{Method validated on maximally challenging test case:} The DHRL oscillator combines rational restoring force $\frac{\lambda x + \mu x^3}{1+\nu x^2}$ with six-term generalized Rayleigh--Li\'enard damping. All analytical predictions agree with fourth-order Runge--Kutta results.
